...
class STARK_ALPHA_SEG_TrackPGD(object):
    ...
    def track(self, img_RGB, prev_msk):
        ...
        '''Step2: Mask report'''
        pred_mask, search, search_mask = self.alpha.get_mask(img_RGB, np.array(pred_bbox), prev_msk, vis=True, trt=self.trt)
...
def run_vot_exp(tracker_name, para_name, refine_model_name, threshold, VIS=False, trt='false'):
    ...
    tracker = STARK_ALPHA_SEG_TrackPGD(tracker_name=tracker_name, para_name=para_name,
    ...
    while True:
        ...
        if not imagefile:
            break
        image = cv2.cvtColor(cv2.imread(imagefile), cv2.COLOR_BGR2RGB)  # Right
        b1, m, search, search_m = tracker.track(image, prev_msk)
        prev_msk = m
        handle.report(m)
        if VIS:
            '''Visualization'''
