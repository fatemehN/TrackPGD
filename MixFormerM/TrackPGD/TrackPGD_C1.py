...
def TrackPGD(net, img, target, template_feats, roi_bb, search_factor, epsilon_ = 10, alpha_= 1, max_t=10):  
    """
    TrackPGD attack for MixFormer tracker
    Args:
        net: AlphaRefine network (segmentation head of MixFormer)
        img: Current frame
    ...
    for t in range(max_t):
        ...
        #Predict the current mask (M^t)
        net = net.cuda()
        pred = net.forward_test(x_adv, template_feats, roi_bb, mode='test')
        msk = pred['mask'].squeeze(dim=0).squeeze(dim=0) #.permute((1, 2, 0))
        ...
        bbox = pred2bbox(pred, input_type='corner')
        #Crop Target mask to generate the cropped ground truth for this step (G_\tau)
        gt = sample_target(target, bbox, search_factor, output_sz=256, mode=cv2.BORDER_REPLICATE)
        ...
        loss_true = (1 - lmbd) * loss_pos + lmbd * loss_neg
        ...
        loss_bck = (1 - lmbd) * loss_pos_alt + lmbd * loss_neg_alt
        #Compute difference loss L_\Delta
        loss_cls = (loss_true - loss_bck) 
        ...
        loss = 10*loss_dice + 2.5*loss_focal
